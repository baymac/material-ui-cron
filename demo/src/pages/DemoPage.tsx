...
export function DemoPage() {
  ...
  // Theme is created below the toggle state so the dark-mode switch can flip
  // `palette.mode`. The Scheduler reads its colors from this theme, so it
        ...
        <Container maxWidth="md" sx={{ py: 5 }}>
          <Stack spacing={1} sx={{ mb: 4 }}>
            ...
            <Typography variant="body1" color="text.secondary">
              Edit the schedule below, try a preset, toggle admin mode, switch the locale, flip
              dark mode, or preview the mobile layout. The component is rendered straight from this
              repository's <code>src/</code>.
            </Typography>
          </Stack>
          ...
          <Paper variant="outlined" sx={{ p: { xs: 2, sm: 3 }, mb: 3 }}>
            ...
            {/* In mobile view the card is constrained to a phone-width frame so
                the Scheduler's container queries kick in (stacked layout, wrapped
                header) — the page viewport is unchanged. */}
            <Box
              sx={
                isMobile
                  ? {
